...
- 当前是规则型 mock judge，只能识别显式模式，不能真正理解所有语义。
- 没有接入订单、商品、物流、账号等真实系统，无法验证外部事实。
- `human_ref.json` 没有数值评分，因此只能做问题类型一致性验证，不能做严格相关系数。
- 规则偏向“主动代办/追问”的客服风格；如果业务策略希望强自助，需要重新调权重。
- 部分 case 可能误判，例如回复给了合理通用规则但没有代办时，系统会偏严格。
